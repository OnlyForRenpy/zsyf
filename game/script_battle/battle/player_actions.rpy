label turn_actions:
    while not battleEnd:
        $ playersCnt()      # 结算有哪些player活着
        $ currentplayer = None
        $ message = "attack"
        call screen turn_select
        $ currentplayer = _return
        if currentplayer != "over":
            call player_skill from _call_player_skill   # 一些特殊效果，如防御等
            $ endTurn()         # 应用伤害和结算
...
screen turn_select():
    ...
    frame:
        ...
        has vbox:
            ...
            textbutton "回合结束" xalign 0.5 action Return("over")
...
label player_skill:
    ...
    $ b_skill = _return
    if isinstance(b_skill, Skill):      # targ和targs在ActiveSkill类里定义
        $ target = b_skill.targ
...
screen choose_skill():
    ...
    add "images/battle/battle/skillbox.png" xcenter 0.5 ycenter 0.5
    # 技能界面
    ...
    viewport:
        ...
        vbox:
            for skll in currentplayer.skills:
                if skll.pwr > 0:
                    textbutton "[skll.name]" text_size 26 action Return(skll), SensitiveIf(sensIf(skll)) tooltip "技能说明：{0}\n伤害: {1}\n消耗: {2}".format(skll.description, skll.pwr, skll.mp_cost)
                elif skll.pwr < 0:
                    textbutton "[skll.name]" text_size 26 action Return(skll), SensitiveIf(sensIf(skll)) tooltip "技能说明：{0}\n回复量: {1}\n消耗: {2}".format(skll.description, -skll.pwr, skll.mp_cost)
                else:
                    textbutton "[skll.name]" text_size 26 action Return(skll), SensitiveIf(sensIf(skll)) tooltip "技能说明：{0}\n消耗: {1}".format(skll.description, skll.mp_cost)
        # for skll in currentplayer.skills:
        #     if skll.img == None:
    ...
    vbox:
        ...
        ypos 250
        spacing 25
        textbutton "普攻" align (.5,.5) action Return("attack")
        textbutton "道具" align (.5,.5) action Return("item")
        textbutton "防御" align (.5,.5) action Return("defend")
        textbutton "取消" align (.5,.5) action Function(renpy.pop_call), Jump("turn_actions")
    # timer 300 action Hide('choose_skill'), Function(renpy.pop_call), Jump("turn_actions")

label target_select(targs=1):
    if target == "enemy":
        $ message = "target_who"
        while targs > 0 and len(picked_targs) < (monsters_total - monsters_dead):   # 目标数量<现存怪物数量，则需选择怪物
...
screen select_monster():
    ...
    frame:
        yalign 0.2
        has vbox:
            textbutton "返回" xalign 0.5 action Function(renpy.pop_call), Jump("player_skill"), SensitiveIf(not picked_targs)

screen monster_dmg():
    style_group "dmg"
    for m in monster_slot:
        if m in missed_t:
            text "[misstext]" color "#0f0" anchor (.5,.5) pos m.dmg_pos at shake_fade
        elif m in skill_t:
...
screen player_dmg():
    ...
    for p in battle_players:
        if p in missed_t:
            text "[misstext]" color "#0f0" anchor (.5,.5) xpos p.dmg_xpos ypos p.dmg_ypos at shake_fade
        elif p in hit_t:
            text "[m_damage]" anchor (.5,.5) xpos p.dmg_xpos ypos p.dmg_ypos at shake_fade
        elif p in skill_t:
            if radar_block:
                # add "radar" anchor (.5,.5) xpos p.dmg_xpos ypos p.dmg_ypos at shake_fade
                text "[skilltext]" color "#000" anchor (.5,.5) xpos p.dmg_xpos ypos p.dmg_ypos at shake_fade
