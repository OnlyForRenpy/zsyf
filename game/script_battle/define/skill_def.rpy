...
init python:
    def canTarget(m):
        ...
        if renpy.get_screen("select_monster") and not m in picked_targs:
            ...
            #     return True
            if monster_slot.index(m) > 2: # if m is in front row
                return True
            ...
            if isinstance(b_skill, Skill):
                if b_skill.back_row: # specific skills
                    return True
            if monster_slot[monster_slot.index(m)+3].dead: # if there's no monster blocking it
                return True
        else:
            return False
    ...
    class Skill(object):
        ...
        def useSkill(self):
            ...
            atk_sfx = "sound/battle_system/battle/skills/" + self.sfx + ".ogg"
            msg_skill = self.name
            s_trans = self.trans
    ...
    class ActiveSkill(Skill):
        def __init__(self, name, pwr, mp_cost, sfx='sword', targ='enemy', targs=1, type='active', trans=None, img=None, dice=[2,8], acc=0, lvl=0, back_row=False, description=""):
            super(Skill, self).__init__()
            self.name = name
            self.pwr = pwr
            self.type = type
